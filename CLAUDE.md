...
python scripts/rating_distribution.py       # v5.19 등급·목표가 편향 (주기적)
...
"5년 평균 대비"/z-score/시그마를 쓰면 B23 이 FAIL 을 낸다.
단 저자가 표현 옆에서 스스로 무효를 밝히면 통과시킨다(정직한 문장을 지우게 하면 안 된다).
실측: 21종목 중 **11종목이 무효**, 그중 **10건이 무효 밴드를 논거로 쓰고 있었다**.
자세한 목록은 `docs/known-report-defects.md`.

> ⚠️ KR 에서는 순이익/EPS 로 주식수를 역산하지 말 것. 연결 순이익은 비지배지분을 포함하고
> EPS 는 지배주주 기준이라 저마진 연도·지주사에서 배수가 어긋난다(두산 4.2배, 에스엠 0.04배).
> 그걸로 연도를 제외하면 분산 큰 해가 지워져 노이즈 밴드가 "유효" 로 뒤집힌다.
...
## ⚠️ v5.15 를 적용할 때 반드시 걸리는 것 두 가지 (v5.17)

기존 리포트 3편을 v5.15 기준으로 고치면서 **같은 함정에 두 번 이상** 빠졌다.

**① 재무·밸류를 채우면 C21 이 깨진다.** v5.15 가 요구하는 현금흐름·DuPont·
DCF·민감도는 전부 재무와 밸류에 들어가는데, C21 은 스토리 3섹션이 본문의
55% 이상이어야 한다. 실측: 한화에어로 55%+ -> **51.4%**, 삼성SDI -> **54.1%**.

필요한 추가 분량은 계산된다 (스토리 S, 본문 T):

```
X >= (0.55 x T - S) / 0.45
```

**깎지 말고 스토리를 채운다.** 재료는 남아 있는 WARN -- `source_coverage` /
`section_rubric` 의 WARN 은 "1차 출처에 있는데 안 쓴 것"이라 채우면 분량과
품질이 같이 오른다. **작성 순서는 스토리 -> 재무·밸류 -> C21 재측정 -> WARN 보강.**

**② s01 에 마진 노트가 없으면 커버 불릿이 1개만 나온다.** 실측 2/3.
...
43개 리포트를 세어 보니 **컨센 대비 목표가 중앙값이 v5.8 까지 -7.7% 였는데
v5.9 이후 -25% 로 벌어졌다.** 컨센 목표가를 확보한 30종목 중 컨센을 넘는
판단은 5건뿐이고 v5.9 이후로는 사실상 0건이다. 등급도 SELL 이 7% -> 62% 로 늘었다.

원인은 버그가 아니라 **좋은 규칙 셋이 전부 같은 방향을 밀기 때문**이다 --
bottom-up(v5.10 규칙 3) · 자본효율 정당화(v5.12 규칙 5) · DCF 병행(v5.15 규칙 3).
셋 다 옳고, 국내 셀사이드 낙관 편향을 감안하면 컨센 아래가 정상일 수 있다.

**규칙을 되돌리지 않는다. 대신 둘을 한다.**

**① 보이게 만든다** -- `python scripts/rating_distribution.py`
버전대별 등급 분포와 컨센 괴리를 내고 쏠림(한 등급 75%+ / 중앙값 |25%|+ /
전 종목 컨센 아래)이면 경고한다. 표본 5 미만이면 경고하지 않는다.

**② 벌어진 만큼 이유를 대게 한다** -- `verify_facts` **gap5**.
컨센 평균과 15% 이상 벌어졌는데 본문에 그 차이를 밝힌 대목이 없으면 FAIL.
**방향 무관**이다. 기존 `gap2`(BUY 가 컨센을 그대로 따라가면 지적)는 취지가
달라 그대로 두었다 -- 국내 컨센이 사실상 전부 매수라 추종 위험은 BUY 쪽에 있다.

> **규칙을 추가할 때마다 "이 규칙은 어느 방향으로 미는가"를 묻는다.**
> 방향이 있으면 반대 방향 검사도 같이 만든다.
