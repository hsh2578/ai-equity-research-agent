...
### 1-2. WebSearch (v4.7부터 **항상 6~8개 최대 병렬 고정**)

**⚠️ v4.7 최적화**: 과거 "4~8개 범위" 지시는 LLM이 4개로 최소화하는 경향이 있어서 이후 정보 부족으로 추가 검색 루프 발생 (평균 1~2분 낭비). **이제부터는 무조건 6~8개 고정 병렬 실행**. 한 메시지에 6~8개 WebSearch tool call 을 동시 호출한다.
...
**검증 방법 -- 자동 스크립트 2개 실행 (v4.9 C 블록 추가)**
