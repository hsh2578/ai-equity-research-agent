"""워드 도표 삽입 v4 — IR 이미지 추가 매핑 (총 64개)"""
import sys, io, re
...
EXACT = [
    ...
    ('YG 지배구조 도식', 'm_ir_yg_cover', 'SK증권 산업·기업분석(2026.03.18) p.64, 위닝펀드'),
    ('매출 부문별 5년 시계열 (음반·공연·음원·기타 stacked', 'company_revenue_5y', 'FnGuide SVD_Finance, YG 사업보고서, 위닝펀드'),
    ('YG 5년 분기별 매출 변동', 'm_mirae_quarterly', '미래에셋증권 기업분석(2026.03.19), 위닝펀드'),
    ('와이지 시스템 개편 비교표', 'm_ir_yg_album_concert', 'SK증권 산업·기업분석(2026.03.18) p.68, 위닝펀드'),
    ('베이비몬스터 유튜브 구독자 추이 (2024.04 데뷔', 'baemon_youtube', 'iM증권(2026.04.15), YouTube, 위닝펀드'),
    ('유튜브 1,000만 달성 소요 시간 비교', 'm_ir_4comp_debut', 'SK증권 산업·기업분석(2026.03.18) p.70, 위닝펀드'),
    ('와이지 IP별 데뷔 후 첫 5개 앨범', 'company_comeback_cycle', '한터차트, YG 사업보고서, 위닝펀드'),
    ('와이지 IP별 평균 컴백 간격 비교', 'company_comeback_cycle', 'YG 사업보고서, 한터차트, 위닝펀드'),
    ('K-POP 4사 메가 IP 양성 능력', 'm_ir_4comp_debut', 'SK증권 산업·기업분석(2026.03.18) p.70, 위닝펀드'),
    ('4사 12M Forward PER vs 메가 IP 보유 수 산점도', 'industry_4comp', 'FnGuide 4사 컨센, 위닝펀드'),
    ('양성형 아이돌 트렌드 비교', 'm_ir_4comp_debut', 'SK증권 산업·기업분석(2026.03.18) p.70, 위닝펀드'),
    ('4사 메가 IP 양성 vs 컴백 효율화 2×2 매트릭스', 'industry_4comp', 'FnGuide 4사 분석, 위닝펀드'),
    ('베이비몬스터 일본 팝업스토어 + MD 라인업 (iM증권', 'analyst_iM_p2', 'iM증권 기업분석(2026.04.15) p.2, 위닝펀드'),
    ('유튜브 구독자수 1,000만+ K팝 아티스트 순위', 'm_ir_4comp_debut', 'SK증권 산업·기업분석(2026.03.18) p.70, 위닝펀드'),
    ('YG 보유 IP 6종 포트폴리오 도식', 'm_ir_yg_album_concert', 'SK증권 산업·기업분석(2026.03.18) p.68, 위닝펀드'),
    ('컴백 주기 비교 (이전 평균 18개월+ vs 현재', 'company_comeback_cycle', '한터차트, 위닝펀드'),
    ('베몬 vs 글로벌 K팝 걸그룹 유튜브 구독자 비교', 'm_ir_4comp_debut', 'SK증권 산업·기업분석(2026.03.18) p.70, 위닝펀드'),
    ('5년 일봉 주가 + 9개 이벤트 마킹', 'm30_5y_events', 'FinanceDataReader 일봉, 위닝펀드'),
    ('베이비몬스터 데뷔 발표 단계별 주가 반응', 'pattern_190', 'FinanceDataReader 일봉, 위닝펀드'),
    ...
    ('YG 보유 활동 IP 수 5년 추이', 'ip_timeline', 'YG 사업보고서, 위닝펀드'),
    ('24.10~25.08 vs 26.08~27.07 사이클 비교 매트릭스', 'm35_cycle_compare', '본 리포트 자체 추정, FnGuide, 위닝펀드'),
    ('시총 추이 비교 (24.10 6,916억', 'm35_cycle_compare', 'FDR 일봉 + 본 리포트 추정, 위닝펀드'),
    # 투자포인트
    ...
    ('빅뱅 코첼라 4월 12·19일 무대 1주차 영상 1,750만뷰', 'm41_coachella', 'YouTube Coachella, iM증권(2026.04.15), 위닝펀드'),
    ...
    ('12개월 카탈리스트 타임라인', 'm_ir_yg_album_concert', 'SK증권 산업·기업분석(2026.03.18) p.68, 위닝펀드'),
    ...
    ('2017~2019 PER 하단 박스권 + 블랙핑크 부상', 'm_mirae_per_pbr_band', '미래에셋증권 기업분석(2026.03.19) p.5, 위닝펀드'),
    # 리스크
    ('블랙핑크 그룹 vs 개별 활동 시기 도식', 'm58_blackpink_solo', 'YG 공시·아티스트 개별 소속사, 위닝펀드'),
    ('텐센트뮤직 지분 평가손익 5년 시계열', 'm60_tencent_pl', '유진투자증권(2026.05.11), Yahoo TME, 위닝펀드'),
    ...
    ('외국인 지분율 12개월 시계열', 'm65_foreign_ratio', 'SK증권 기업분석(2026.05.11), KIS API, 위닝펀드'),
    ...
    ('2026E ROE 10.8% 회복 경로', 'm70_roe_recovery', '한화투자증권(2026.04.13), 위닝펀드'),
    # 밸류에이션
    ('19개 증권사 Implied Target PER', 'm71_consensus_dist', '교보증권(2026.05.11), WiseReport, 위닝펀드'),
    ...
    ('89,000원 도달 시점 트리거 타임라인', 'm88_price_ladder', '본 리포트 자체 산정, 위닝펀드'),
    ...
    ('적정 시총 16,600억 구성', 'cash_bridge', 'FnGuide + 본 리포트, 위닝펀드'),
    ('IP × 분기 매출 시간표 매트릭스', 'm83_ip_quarterly', '본 리포트 IP 라이프사이클, 위닝펀드'),
    ('24.10~25.08 +190% 사이클 vs 26.08~27.07 예상', 'm35_cycle_compare', '본 리포트 + FDR 일봉, 위닝펀드'),
    ("'양성 시스템 검증 매트릭스'", 'm86_system_validation', '본 리포트 자체 정리, 베몬 실측 + 산업 평균, 위닝펀드'),
    ('9월 보이그룹 데뷔 후 점검 시점', 'm62_sept_scenario', '본 리포트 자체 시나리오, 위닝펀드'),
