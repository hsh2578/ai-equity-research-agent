"""워드 도표 매핑 v5 — 정밀 1:1, 중복 제거, 부적합 텍스트 페이지 제외"""
import sys, io, re
...
EXACT = [
    ...
    ('2017~2019 PER 하단 박스권', 'm_mirae_per_pbr_band', '미래에셋증권 기업분석(2026.03.19) p.5, 위닝펀드'),
    # --- 리스크 (3개) ---
...
def main():
    ...
    text_only = 0
    for idx, p in memos:
        png, kw, src = find_match(p.text, used_pngs)
        cap = short(p.text)
        if png:
    ...
    print(f'PNG + 출처 삽입: {inserted}개 (모두 1:1 매핑, 중복 0)')
    print(f'텍스트 메모 유지: {text_only}개')
    print(f'사용된 고유 PNG: {len(used_pngs)}개')
    doc.save(str(DOC))
